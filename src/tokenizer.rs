use std::{iter::Enumerate, str::Chars, vec, process::exit};
use colour::red;
#[derive(Debug)]
#[derive(PartialEq, Eq)]
pub enum Tokens{
        ...
        char_iter.for_each(|ch|{
            if skip == false {
                match ch.1 {
                    ...
                    _ => {
                        red!("Syntax Error: ");
                        println!("Unexpected token at line {} character {}.
        {}| {}
           {}^-- Unexpected '{}' token.", line.0 + 1, ch.0 + 1, line.0 + 1, line.1, " ".repeat(ch.0), ch.1);
           exit(1)
                    }
                    }  
        }else{
...
mod tests{
    ...
    fn correct_tokenization(){
        ...
        assert_eq!(tokenize("()".to_string()), vec!(Tokens::LPAREN, Tokens::RPAREN))
    }
}